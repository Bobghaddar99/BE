...
class EditRoomDialog(QDialog):
    ...
    def update_room_data(self):
        ...
        pose_text = "X: {}, Y: {}".format(pose.pose.position.x, pose.pose.position.y)
...
class AutomappingPage(QWidget):
    ...
    def process_map_name(self,input_field_map):
        ...
        output_dir = r"/home/jetchair/Desktop/be/src/maps/map_saved" # Change this to your preferred directory
    ...
    # Run the map_saver command to save the map
        try:
            ...
            # Check the result and log appropriate messag
    ...
    def start_automapping(self):
        ...
        subprocess.Popen(["roslaunch", "jet_main", "launcher.launch"])
...
class VisualizationPage(QWidget):
    ...
    def launch_rviz(self):
        rviz_config_path = "/home/jetchair/Desktop/gui_test/src/gui_ros/src/config_gui_rviz.rviz"  # Update this path
...
class MapsLoader(QWidget):
    def __init__(self, dashboard_gui, maps_directory=r"/home/jetchair/Desktop/be/src/maps/map_saved"):
        super(MapsLoader, self).__init__()
        self.dashboard_gui = dashboard_gui
        self.maps_directory = os.path.expanduser(maps_directory)
        layout = QVBoxLayout(self)

        # Label
        self.label = QLabel("Select a Map", self)
        self.label.setStyleSheet("font-size: 20px; color: #323232; font-family: 'Roboto', sans-serif;")
        self.label.setAlignment(Qt.AlignCenter)
        layout.addWidget(self.label)
        ...
        self.launch_button = QPushButton("Launch Map", self)
        self.launch_button.clicked.connect(self.launch_selected_map)
        layout.addWidget(self.launch_button, alignment=Qt.AlignCenter)

        self.setLayout(layout)

    def load_maps(self):
        """Load all .yaml map files from the directory into the dropdown menu."""
        if not os.path.exists(self.maps_directory):
            os.makedirs(self.maps_directory)  # Create directory if it doesn't exist
        self.map_selector.clear()
        maps = [f for f in os.listdir(self.maps_directory) if f.endswith('.yaml')]

        if not maps:
            rospy.logwarn("No maps found in the directory.")
            self.map_selector.addItem("No maps available")
        else:
            self.map_selector.addItems(maps)

    def launch_selected_map(self):
        """Set ROS param and launch an application with the selected map."""
        selected_map = self.map_selector.currentText()
        if selected_map == "No maps available":
            rospy.logwarn("No valid map selected.")
            return

        map_path = os.path.join(self.maps_directory, selected_map)

        # Set ROS parameter
        rospy.set_param("/map_file", map_path)

        # Launch RViz or another node (modify as needed)
        try:s
            subprocess.Popen(["roslaunch", "jet_main", "launcher_saved.launch", "map_file:=" + map_path])
            rospy.loginfo("Launching map visualizer...")
        except Exception as e:
            rospy.logerr("Failed to launch visualization: ")
...
class DashboardGUI(QMainWindow):
    def __init__(self):
        ...
        self.maps = StyledButton("maps", None, self)
        ...
        self.maps.clicked.connect(self.load_map)
        ...
        side_menu_layout.addWidget(self.maps)
        ...
        self.MapsLoader = MapsLoader(self)
        ...
        self.stacked_widget.addWidget(self.visualization_page)
        self.stacked_widget.addWidget(self.MapsLoader)
        ...
        self.auto_mapping_button = StyledButton("Auto Mapping", "/home/jetchair/Desktop/gui_test/src/gui_ros/src/icons_gui/robot.png", self)
        self.docking_button = StyledButton("Docking", "/home/jetchair/Desktop/gui_test/src/gui_ros/src/icons_gui/home.png", self)
        self.lift_button = StyledButton("Lift", "/home/jetchair/Desktop/gui_test/src/gui_ros/src/icons_gui/lifting.png", self)
        self.choose_rooms_button = StyledButton("Choose Rooms", "/home/jetchair/Desktop/gui_test/src/gui_ros/src/icons_gui/press-button.png", self)
        self.emergent_button = StyledButton("Emergencay","/home/jetchair/Desktop/gui_test/src/gui_ros/src/icons_gui/emerg.jpg",self)
        self.joystick = StyledButton("joystick","/home/jetchair/Desktop/gui_test/src/gui_ros/src/icons_gui/emerg.jpg",self)
        ...
        # Add buttons to the main page layout (grid)jetchair
    ...
    def show_visualization_page(self):
        ...
        self.hide_menu()
    def load_map(self):
        self.stacked_widget.setCurrentWidget(self.MapsLoader)
        self.hide_menu()
